...
// column.
$column_classes = [NULL, 'col-md-12 col-sm-12', NULL];
...
elseif (!$field_co_cntct_logo && $field_co_cntct_map) {
  ...
  $column_classes = [NULL, 'col-md-7 col-sm-12', 'col-md-5 col-sm-12'];
}
elseif ($field_co_cntct_logo && !$field_co_cntct_map) {
  // Logo but no map.
  $column_classes = ['col-md-4 col-sm-4', 'col-md-8 col-sm-8', NULL];
}

?>

<div class="paragraphs-item paragraphs-item--company-contact-info container">
  <div class="row">
            ...
            <li class="contact-link contact-link-type-<?php print $link_entity['field_co_cntct_fg_type'][0]['#markup'] ?>">
              ...
              <?php switch ($link_entity['field_co_cntct_fg_type'][0]['#markup']) {
                ...
                <?php case 'url': ?>
                  ...
                  <?php print l($url_friendly, $link_entity['field_co_cntct_fg_url'][0]['#markup'], ['absolute' => TRUE]) ?>

                  <?php break ?>

                <?php case 'address': ?>
                  <?php // For addresses, we create a link to Google Maps. ?>
                  <?php print l($link_entity['field_co_cntct_fg_address'][0]['#markup'], 'https://maps.google.com?q=' . urlencode($link_entity['field_co_cntct_fg_address'][0]['#markup']), ['absolute' => TRUE]) ?>
                  <?php break ?>
